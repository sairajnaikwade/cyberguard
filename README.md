# CyberGuard — Advanced Network Vulnerability Scanner

> A production-ready, full-stack network vulnerability scanner and threat intelligence platform built with Python, Flask, Celery, and Nmap.
...
* **Nmap-based port and service scanning**: Performs rapid and detailed target scanning wrapping the system-level Nmap binary with support for Quick, Full, and Vuln script scan configurations.
* **Real-time CVE enrichment using NIST NVD API**: Dynamically queries the NIST National Vulnerability Database API to pull up-to-date CVE information, descriptions, and CVSS v3.1/v3.0/v2 scoring metrics.
* **Shodan Threat Intelligence Integration**: Gathers deep threat intelligence (organization, ISP, ASN, geo-location, and public-facing open ports) on resolved public IPs.
* **CVSS-based risk scoring**: Calculates a composite security risk index (0.0 - 10.0) based on detected CVE severity and open port count.
* **User authentication and authorization**: Fully secure user registry with password hashing and session management.
* **Admin dashboard**: Platform management panel for system administrators to view real-time statistics, promote/demote administrators, and clean up scans.
* **Scheduled recurring scans using APScheduler**: Supports user-defined scan intervals (e.g., daily, weekly) automatically managed and triggered via a background process.
* **Email alerts for Critical CVEs**: Dispatches automated SMTP alerts when critical severity vulnerabilities (CVSS >= 9.0) are found.
* **Celery + Redis background task processing**: Offloads intensive scanning workloads to isolated Celery worker processes, preventing main server thread blockages.
* **CSV and PDF report generation**: Exports beautifully formatted tabular CSVs and professional dark-themed PDF reports featuring custom table formatting via ReportLab.
* **Flask-Limiter rate limiting**: Implements route-specific rate limits to defend authentication and scan paths against brute-force and resource abuse.
* **Docker deployment**: Multi-container Docker Compose definition ensuring seamless local and staging environment deployments.
* **GitHub Actions CI/CD**: Automated workflow testing integration, build verification, and coverage analysis.
* **Pytest test suite with 99% coverage**: Comprehensive test suite verifying core business logic, controllers, database operations, and external API mocks.

---

## Tech Stack

* **Core Framework**: Python 3.12 / 3.13, Flask 3, Flask-Login
* **Asynchronous Tasks**: Celery, Redis
* **Task Scheduling**: APScheduler
* **Database & Migrations**: SQLite (dev) / PostgreSQL (production), SQLAlchemy, Flask-Migrate
* **Scanner Core**: python-nmap (System Nmap wrapper)
* **Security & Controls**: Flask-Limiter, Werkzeug Security (Scrypt Hashing)
* **Threat Intel & CVEs**: NIST NVD REST API v2, Shodan API
* **Reporting**: ReportLab (PDF), Standard CSV Library
* **Frontend**: Vanilla HTML5, CSS3 (CSS Variables, Flexbox, Grid, Glassmorphism), JavaScript (Vanilla ES6, Live Polling)
* **CI/CD & Virtualization**: GitHub Actions, Docker, Docker Compose
* **Testing Suite**: Pytest, Pytest-Cov, Freezegun (time mocking)
...
* **Dashboard**: `[Placeholder: Glassmorphic dark theme dashboard panel showing metrics grid and new scan trigger form]`
* **Scan Results**: `[Placeholder: Detailed scan results table mapping open ports, matched CVEs, CVSS ratings, and Shodan geolocation data]`
* **Admin Panel**: `[Placeholder: Admin panel view showing total system statistics, scan removal actions, and role toggle buttons]`
* **Scheduled Scans**: `[Placeholder: UI list displaying configured targets, hourly intervals, status switches, and next run calculations]`
* **PDF Report**: `[Placeholder: Clean, high-contrast dark PDF document layout displaying professional security report layout]`
...
* **Engineered a production-ready asynchronous network vulnerability scanner** utilizing **Flask 3**, **SQLAlchemy**, and **Celery** with a **Redis** broker, allowing concurrent Nmap scans to execute in background tasks without blocking main web server thread resources.
